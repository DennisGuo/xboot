...
(
    ...
    pid         varchar references t_group on delete cascade
);
...
(
    user_id     VARCHAR NOT NULL REFERENCES t_user ON DELETE CASCADE,
    group_id    VARCHAR NOT NULL REFERENCES t_group ON DELETE CASCADE,
    create_by   varchar,
    update_by   varchar,
    create_time timestamp default CURRENT_TIMESTAMP,
    update_time TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT t_group_user_un UNIQUE (user_id, group_id)
);
...
(
    ...
    create_time timestamp default CURRENT_TIMESTAMP,
    update_time timestamp default CURRENT_TIMESTAMP,
    dict_id     varchar not null references t_dict on delete cascade,
    dict_code   varchar,
    code        varchar,
    sort        int       default 0,
    value       text
...
(
    ...
    create_time timestamp default CURRENT_TIMESTAMP,
    update_time timestamp default CURRENT_TIMESTAMP,
    role_id     varchar references t_role on delete cascade,
    menu_id     varchar references t_menu on delete cascade,
    constraint t_role_menu_un unique (role_id, menu_id)
);
